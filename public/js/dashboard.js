...
const CONFIG = {
    ...
    STORAGE_KEYS: { USER: 'nimwema_user' } // Minimal—use cookies for auth
};
...
document.addEventListener('DOMContentLoaded', function() {
  if (checkAuth()) {
    loadUserData();
    showSection('pending-orders'); // Default to pending
    startRefresh();
  }
});
...
async function checkAuth() {
  try {
    const response = await fetch(`${CONFIG.API_BASE}/auth/me`, { credentials: 'include' });
    ...
    if (userNameEl) {
      userNameEl.textContent = `${currentUser.firstName || ''} ${currentUser.lastName || ''}`.trim() || 'Utilisateur';
    }
    ...
    window.location.href = '/login.html'; // Redirect on fail
    return false;
  }
}
...
async function loadUserData() {
  ...
  Promise.allSettled([
    ...
    console.log('📊 Dashboard loaded:', results.map(r => r.status));
  });
}

// Show Section (nav handler)
...
// ============================================
// TRANSACTIONS SECTION (Vouchers + Orders History)
// ============================================
